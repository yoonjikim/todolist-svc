...
//   try {
//     let count = Todo.countDocuments({ owner:owner});
//     if (count === 0) {
...
//       await todo1.save(); // save : 데이터를 저장하는 메소드
//       await todo2.save();
//       await todo3.save();
//     } else {
//       return { status: "fail", message: "생성 실패 : 이미 존재하는 owner입니다." + ex };
//     }
//   } catch (ex) {
...
// // 검색 + _id 형식 변경(mongodb 티 안나게)
// exports.getTodoList = async ({ owner }) => {
//   try {
//     let todolist = await Todo.find({ owner }).sort({ _id:-1 });
//     todolist = todolist.map((t) => {
//       let { _id, user_id, todo, desc, completed } = t;
//       return { id:_id, user_id, todo, desc, completed };
//     })
